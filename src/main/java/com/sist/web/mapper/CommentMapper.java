...
public interface CommentMapper {
  ...
  @Select("SELECT no,cno,type,id,name,msg,"
	    ...
	    +"ORDER BY no DESC")
  public List<CommentVO> commentListData(@Param("cno") Integer cno,
		  @Param("type") Integer type);
  // 추가
  // SEQUENCE => before : 먼저 수행 
  @SelectKey(keyProperty = "no",resultType = int.class,before = true,
		  statement = "SELECT NVL(MAX(no)+1,1) FROM comment_0")
  @Insert("INSERT INTO comment_0 VALUES("
		 +"#{no},#{cno},#{type},#{id},#{name},#{msg},SYSDATE)")
  public void commentInsert(CommentVO vo);
  // 수정 
  @Update("UPDATE comment_0 SET "
		 +"msg=#{msg} "
		 +"WHERE no=#{no}")
  public void commentUpdate(@Param("no") Integer no,
		  @Param("msg") String msg);
  // 삭제 
  @Delete("DELETE FROM comment_0 "
		 +"WHERE no=#{no}")
